...
class FirebaseService {
  ...
  static removeLocalItem(Item item) {
    ...
    Outfit.outfitList.removeWhere((outfit) =>
        outfit.topItem.id == item.id ||
        outfit.bottomItem.id == item.id ||
        outfit.shoeItem.id == item.id);
    //removes item locally
  ...
  static editFirestoreItemDetails(
      Item item, String label, String category, List<String> weather) {
    final itemToSet = {
    ...
    db
        ...
        .collection('Clothes')
        .doc(item.id.toString())
        .set(itemToSet, SetOptions(merge: true));
    return 0;
  }

  //edits item in local Item.itemList
  static editLocalItemDetails(
      Item item, String label, String category, List<String> weather) {
    if (label != '') {
      item.label = label;
    }
  ...
  static addFirestoreOutfit(String label, int id, Item top, Item bottom,
      Item shoes, int timesWorn, List<String> weather) {
    final outfitStorage = {
      'label': label,
      'id': id,
      'topID':
          top.id, // updated to topID to match what Outfit.fromFirestore expects
      'bottomID': bottom
          .id, // updated to bottomID to match what Outfit.fromFirestore expects
      'shoesID': shoes
          .id, // updated to shoesID to match what Outfit.fromFirestore expects
      'timesWorn': timesWorn,
      'weather': weather,
    };
  ...
  //add outfit locally
  static addLocalOutfit(String category, int id, Item top, Item bottom,
      Item shoes, int timesWorn, List<String> weather) {
  ...
  static Future<bool> isOutfitFavorited(
      int topID, int bottomID, int shoeID) async {
    final userDoc =
        FirebaseFirestore.instance.collection('users').doc(kUsername);
    final favorites = await userDoc
